  ...
  describe "total revenue by date range" do
    describe "happy path" do
      it "gives the total revenue of the whole system given a date range" do
        ...
        expect(result[:properties]).to have_key(:data)
        expect(result[:properties][:data]).to have_key(:properties)
        expect(result[:properties][:data][:properties]).to be_a(Hash)
        expect(result[:properties][:data][:properties]).to have_key(:attributes)
        expect(result[:properties][:data][:properties][:attributes]).to be_a(Hash)
        expect(result[:properties][:data][:properties][:attributes]).to have_key(:properties)
        expect(result[:properties][:data][:properties][:attributes][:properties]).to be_a(Hash)
        expect(result[:properties][:data][:properties][:attributes][:properties]).to have_key(:revenue)
      end
    end
  end
end